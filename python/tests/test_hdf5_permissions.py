...
def test_read_with_writable_master_and_readonly_ids():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        master_path = os.path.join(tmpdir, "master.h5")
        ids_path = os.path.join(tmpdir, "equilibrium.h5")
        ...
        result = _read_ids(tmpdir)
        assert result is not None


def test_write_to_readonly_ids_raises():
...
def test_read_with_readonly_master_and_readonly_ids():
...
def test_put_with_readonly_master_and_existing_rw_ids():
...
def test_put_with_readonly_master_and_no_ids_link_raises():
