...
def _al_write_data_scalar(ctx, fieldname, pyTimebasePath,  inputData, dataType):
    ...
    if al_status.code < 0:
        raise get_proper_exception_class(f'Error while writing data: {al_status.message}', al_status.code)
...
def al_read_data_scalar(ctx, fieldPath, pyTimebasePath, dataType):
    ...
    if al_status.code < 0:
        raise get_proper_exception_class(f'Error while reading data: {al_status.message}', al_status.code)
...
def al_register_plugin(name):
    ...
    if al_status.code < 0:
        raise get_proper_exception_class(f'Error while registering plugin: {al_status.message}', al_status.code)
    return al_status.code

def al_unregister_plugin(name):
    al_status = ll.al_unregister_plugin(name.encode('UTF-8'))
    if al_status.code < 0:
        raise get_proper_exception_class(f'Error while unregistering plugin: {al_status.message}', al_status.code)
    return al_status.code

def al_bind_plugin(path, name):
    al_status = ll.al_bind_plugin(path.encode('UTF-8'), name.encode('UTF-8'))
    if al_status.code < 0:
        raise get_proper_exception_class(f'Error while binding plugin: {al_status.message}', al_status.code)
    return al_status.code

def al_unbind_plugin(path, name):
    al_status = ll.al_unbind_plugin(path.encode('UTF-8'), name.encode('UTF-8'))
    if al_status.code < 0:
        raise get_proper_exception_class(f'Error while unbinding plugin: {al_status.message}', al_status.code)
    return al_status.code

def al_bind_readback_plugins(ctx):
    al_status = ll.al_bind_readback_plugins(ctx)
    if al_status.code < 0:
       raise get_proper_exception_class(f'Error while binding readback plugins: {al_status.message}', al_status.code)
    return al_status.code

def al_unbind_readback_plugins(ctx):
    al_status = ll.al_unbind_readback_plugins(ctx)
    if al_status.code < 0:
        raise get_proper_exception_class(f'Error while unbinding readback plugins: {al_status.message}', al_status.code)
    return al_status.code

def al_write_plugins_metadata(ctx):
    al_status = ll.al_write_plugins_metadata(ctx)
    if al_status.code < 0:
        raise get_proper_exception_class(f'Error while storing plugins metadata: {al_status.message}', al_status.code)
    return al_status.code
...
def al_setvalue_parameter_plugin(parameter_name, inputData, pluginName):
    ...
    if al_status.code < 0:
        raise get_proper_exception_class(f'Error while setting parameter value for plugin: {al_status.message}', al_status.code)
    return al_status.code
...
def al_get_occurrences(ctx, ids_name):
    ...
    if al_status.code < 0:
        raise get_proper_exception_class(f'Error while calling al_get_occurrences: {al_status.message}', al_status.code)
