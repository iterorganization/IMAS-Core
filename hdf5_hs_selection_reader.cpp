...
HDF5HsSelectionReader::HDF5HsSelectionReader(hid_t dataset_id_, hid_t datatype_, int AOSRank_, int *dim):dataset_id(dataset_id_), immutable(true), datatype(datatype_), dataset_rank(-1), AOSRank(AOSRank_), dtype_id(-1), dataspace(-1), memspace(-1), buffer_size(0)
{
    ...
    if (status < 0) {
        ...
        sprintf(error_message, "Unable to call H5Sget_simple_extent_dims to space id: %d\n", (int) dataspace);
...
{
    //memcpy (size, size_, dim * sizeof (int));
    for (int i = 0; i < dim; i++) {
        size[i] = size_[i];
    }
    setBufferSize();
...
void HDF5HsSelectionReader::allocateBuffer(void **data, int slice_mode, bool is_dynamic, bool isTimed, int slice_index)
{
