...
## introspection, #3 round-trip matrix, #6 capability-gated, #4 structured data)

The suite covers the **data-path, introspection, and structured-data surface**
end to end and pins the defects those issues surfaced (sixteen xfails, each
with a paired current-behavior tripwire). It does **not yet** cover any of the
plugin surface or the ownership sweep — those are the remaining build-order
...
current-behavior tripwire, so the xfail can't rot." All sixteen xfail rows
