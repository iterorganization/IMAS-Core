...
static int putSliceLocal(int expIdx, char *cpoPath, char *path, struct descriptor *dataD, double time);
...
//Status reporting:
//0: success
//!= 0 error index. Routine euitm_last_errmsg returns the error message associated with the last error.
extern char *errmsg;
...
//reportInfo("LOCKED CPO\n", "");
}
static void unlockCpo()
{
    pthread_mutex_unlock(&mutexCpo);
//reportInfo("CPO UNLOCK\n", "");
}
...
///////Local-remote stuff
static int mdsBeginCPOPutSliceLocal(int expIdx, char *path);

static int mdsEndCPOPutSliceLocal(int expIdx, char *path); 
static int mdsBeginCPOReplaceLastSliceLocal(int expIdx, char *path);
static int mdsEndCPOReplaceLastSliceLocal(int expIdx, char *path);
static int mdsBeginCPOPutLocal(int expIdx, char *path);
static int mdsEndCPOPutLocal(int expIdx, char *path);
static int mdsBeginCPOPutTimedLocal(int expIdx, char *path, int samples, double *inTimes);
static int mdsEndCPOPutTimedLocal(int expIdx, char *path);
static int mdsBeginCPOPutNonTimedLocal(int expIdx, char *path);
static int mdsEndCPOPutNonTimedLocal(int expIdx, char *path);
static int mdsBeginCPOGetLocal(int expIdx, char *path, int isTimed, int *retSamples);
static int mdsEndCPOGetLocal(int expIdx, char *path);
static int mdsBeginCPOGetSliceLocal(int expIdx, char *path, double time);
static int mdsEndCPOGetSliceLocal(int expIdx, char *path);
...
static void updateInfoTimedObject(int expIdx, char *cpoPath, char *path, int exists, void *obj)
{
    ...
    {
	printf("INTERNAL ERROR in updateInfoTimedObject:Not an APD at path %s, CPO path %s\n", path, cpoPath);
	return;
    }
...
int putSlice(int expIdx, char *cpoPath, char *path, struct descriptor *dataD, double time)
{
    int exists, nDims, dims[16], status;
    EMPTYXD(dataXd);
    EMPTYXD(timeXd);
...
//First get Data, if any so that it is correctly posted in the memory cache
    ...
    if(isExpRemote(expIdx))
    	...
    	return putSliceLocal(expIdx, cpoPath, path, dataD, time);
}
...
int mdsBeginCPOPutSlice(int expIdx, char *path)
{
//reportInfo("BEGIN CPO PUT SLICE %s\n", path);
    lockCpo();
    if(isExpRemote(expIdx))
        return mdsBeginCPOPutSliceRemote(getExpConnectionId(expIdx), getExpRemoteIdx(expIdx), path);
    else
        return mdsBeginCPOPutSliceLocal(expIdx, path);
}

int mdsEndCPOPutSlice(int expIdx, char *path) 
{
//reportInfo("END CPO PUT SLICE %s\n", path);
    int status;
    if(isExpRemote(expIdx))
        status = mdsEndCPOPutSliceRemote(getExpConnectionId(expIdx), getExpRemoteIdx(expIdx), path);
    else
        status = mdsEndCPOPutSliceLocal(expIdx, path);
    unlockCpo();
    return status;
}

int mdsBeginCPOReplaceLastSlice(int expIdx, char *path)
{
    lockCpo();
    if(isExpRemote(expIdx))
        return mdsBeginCPOReplaceLastSliceRemote(getExpConnectionId(expIdx), getExpRemoteIdx(expIdx), path);
    else
        return mdsBeginCPOReplaceLastSliceLocal(expIdx, path);
}

int mdsEndCPOReplaceLastSlice(int expIdx, char *path) 
{
    int status;
    if(isExpRemote(expIdx))
        status = mdsEndCPOReplaceLastSliceRemote(getExpConnectionId(expIdx), getExpRemoteIdx(expIdx), path);
    else
        status = mdsEndCPOReplaceLastSliceLocal(expIdx, path);
    unlockCpo();
    return status;
}

int mdsBeginCPOPut(int expIdx, char *path)
{
//reportInfo("BEGIN CPO PUT  %s\n", path);
    lockCpo();
    if(isExpRemote(expIdx))
        return mdsBeginCPOPutRemote(getExpConnectionId(expIdx), getExpRemoteIdx(expIdx), path);
    else
        return mdsBeginCPOPutLocal(expIdx, path);
}

int mdsEndCPOPut(int expIdx, char *path)
{
//reportInfo("END CPO PUT %s\n", path);
    int status;
    if(isExpRemote(expIdx))
        status = mdsEndCPOPutRemote(getExpConnectionId(expIdx), getExpRemoteIdx(expIdx), path);
    else
        status = mdsEndCPOPutLocal(expIdx, path);
    unlockCpo();
    return status;
}

int mdsBeginCPOPutTimed(int expIdx, char *path, int samples, double *inTimes)
{
//reportInfo("BEGIN CPO PUT TIMED %s\n", path);
    ...
    if(isExpRemote(expIdx))
        return mdsBeginCPOPutTimedRemote(getExpConnectionId(expIdx), getExpRemoteIdx(expIdx), path, samples, inTimes);
    else
        return mdsBeginCPOPutTimedLocal(expIdx, path, samples, inTimes);
}

int mdsEndCPOPutTimed(int expIdx, char *path) 
{
//reportInfo("END CPO PUT TIMED %s\n", path);
    ...
    if(isExpRemote(expIdx))
        status = mdsEndCPOPutTimedRemote(getExpConnectionId(expIdx), getExpRemoteIdx(expIdx), path);
    else
        status = mdsEndCPOPutTimedLocal(expIdx, path);
    unlockCpo();
    return status;
}

int mdsBeginCPOPutNonTimed(int expIdx, char *path)
{
//reportInfo("BEGIN CPO PUT NON TIMED %s\n", path);
    lockCpo();
    if(isExpRemote(expIdx))
        return mdsBeginCPOPutNonTimedRemote(getExpConnectionId(expIdx), getExpRemoteIdx(expIdx), path);
    else
        return mdsBeginCPOPutNonTimedLocal(expIdx, path);
}

int mdsEndCPOPutNonTimed(int expIdx, char *path) 
{
//reportInfo("END CPO PUT NON TIMED %s\n", path);
    int status;
    if(isExpRemote(expIdx))
        status = mdsEndCPOPutNonTimedRemote(getExpConnectionId(expIdx), getExpRemoteIdx(expIdx), path);
    else
        status = mdsEndCPOPutNonTimedLocal(expIdx, path);
    unlockCpo();
    return status;
}
...
int mdsBeginCPOGet(int expIdx, char *path, int isTimed, int *retSamples)
{
    int status;
//reportInfo("BEGIN CPO GET %s\n", path);
    lockCpo();
    if(isExpRemote(expIdx))
        status = mdsBeginCPOGetRemote(getExpConnectionId(expIdx), getExpRemoteIdx(expIdx), path, isTimed, retSamples);
    else
        status = mdsBeginCPOGetLocal(expIdx, path, isTimed, retSamples);
    if(status)
	*retSamples = 0;
    return status;
}

int mdsEndCPOGet(int expIdx, char *path)
{
//reportInfo("END CPO GET %s\n", path);
    int status;
    if(isExpRemote(expIdx))
        status = mdsEndCPOGetRemote(getExpConnectionId(expIdx), getExpRemoteIdx(expIdx), path);
    else
        status = mdsEndCPOGetLocal(expIdx, path);
    unlockCpo();
    return status;
}

int mdsBeginCPOGetSlice(int expIdx, char *path, double time)
{
//reportInfo("BEGIN CPO GET SLICE %s\n", path);
    lockCpo();
    if(isExpRemote(expIdx))
        return mdsBeginCPOGetSliceRemote(getExpConnectionId(expIdx), getExpRemoteIdx(expIdx), path, time);
    else
        return mdsBeginCPOGetSliceLocal(expIdx, path, time);
}

int mdsEndCPOGetSlice(int expIdx, char *path)
{
//reportInfo("END CPO GET SLICE %s\n", path);
    int status;
    if(isExpRemote(expIdx))
        status = mdsEndCPOGetSliceRemote(getExpConnectionId(expIdx), getExpRemoteIdx(expIdx), path);
    else
        status = mdsEndCPOGetSliceLocal(expIdx, path);
    unlockCpo();
    return status;
}
...
static void setDataEnv(char *user, char *tokamak, char *version)
{
  //MERGE
    ...
    */
    ...
    {
      //MERGE
    	sprintf(treeBase0, "/pfs/itmdb/itm_trees/public/%s/%s/mdsplus/0;/pfs/itmdb/itm_trees/public/models/%s/mdsplus", tokamak, version, version);
    	sprintf(treeBase1, "/pfs/itmdb/itm_trees/public/%s/%s/mdsplus/1;/pfs/itmdb/itm_trees/public/models/%s/mdsplus", tokamak, version, version);
    	sprintf(treeBase2, "/pfs/itmdb/itm_trees/public/%s/%s/mdsplus/2;/pfs/itmdb/itm_trees/public/models/%s/mdsplus", tokamak, version, version);
    	sprintf(treeBase3, "/pfs/itmdb/itm_trees/public/%s/%s/mdsplus/3;/pfs/itmdb/itm_trees/public/models/%s/mdsplus", tokamak, version, version);
    	sprintf(treeBase4, "/pfs/itmdb/itm_trees/public/%s/%s/mdsplus/4;/pfs/itmdb/itm_trees/public/models/%s/mdsplus", tokamak, version, version);
    	sprintf(treeBase5, "/pfs/itmdb/itm_trees/public/%s/%s/mdsplus/5;/pfs/itmdb/itm_trees/public/models/%s/mdsplus", tokamak, version, version);
    	sprintf(treeBase6, "/pfs/itmdb/itm_trees/public/%s/%s/mdsplus/6;/pfs/itmdb/itm_trees/public/models/%s/mdsplus", tokamak, version, version);
    	sprintf(treeBase7, "/pfs/itmdb/itm_trees/public/%s/%s/mdsplus/7;/pfs/itmdb/itm_trees/public/models/%s/mdsplus", tokamak, version, version);
    	sprintf(treeBase8, "/pfs/itmdb/itm_trees/public/%s/%s/mdsplus/8;/pfs/itmdb/itm_trees/public/models/%s/mdsplus", tokamak, version, version);
    	sprintf(treeBase9, "/pfs/itmdb/itm_trees/public/%s/%s/mdsplus/9;/pfs/itmdb/itm_trees/public/models/%s/mdsplus", tokamak, version, version);
	/*
    	sprintf(treeBase0, "MDSPLUS_TREE_BASE_0=/pfs/itmdb/itm_trees/public/%s/%s/mdsplus/0;/pfs/itmdb/itm_trees/public/models/%s/mdsplus", tokamak, version, version);
    	sprintf(treeBase1, "MDSPLUS_TREE_BASE_1=/pfs/itmdb/itm_trees/public/%s/%s/mdsplus/1;/pfs/itmdb/itm_trees/public/models/%s/mdsplus", tokamak, version, version);
    	sprintf(treeBase2, "MDSPLUS_TREE_BASE_2=/pfs/itmdb/itm_trees/public/%s/%s/mdsplus/2;/pfs/itmdb/itm_trees/public/models/%s/mdsplus", tokamak, version, version);
    	sprintf(treeBase3, "MDSPLUS_TREE_BASE_3=/pfs/itmdb/itm_trees/public/%s/%s/mdsplus/3;/pfs/itmdb/itm_trees/public/models/%s/mdsplus", tokamak, version, version);
    	sprintf(treeBase4, "MDSPLUS_TREE_BASE_4=/pfs/itmdb/itm_trees/public/%s/%s/mdsplus/4;/pfs/itmdb/itm_trees/public/models/%s/mdsplus", tokamak, version, version);
    	sprintf(treeBase5, "MDSPLUS_TREE_BASE_5=/pfs/itmdb/itm_trees/public/%s/%s/mdsplus/5;/pfs/itmdb/itm_trees/public/models/%s/mdsplus", tokamak, version, version);
    	sprintf(treeBase6, "MDSPLUS_TREE_BASE_6=/pfs/itmdb/itm_trees/public/%s/%s/mdsplus/6;/pfs/itmdb/itm_trees/public/models/%s/mdsplus", tokamak, version, version);
    	sprintf(treeBase7, "MDSPLUS_TREE_BASE_7=/pfs/itmdb/itm_trees/public/%s/%s/mdsplus/7;/pfs/itmdb/itm_trees/public/models/%s/mdsplus", tokamak, version, version);
    	sprintf(treeBase8, "MDSPLUS_TREE_BASE_8=/pfs/itmdb/itm_trees/public/%s/%s/mdsplus/8;/pfs/itmdb/itm_trees/public/models/%s/mdsplus", tokamak, version, version);
    	sprintf(treeBase9, "MDSPLUS_TREE_BASE_9=/pfs/itmdb/itm_trees/public/%s/%s/mdsplus/9;/pfs/itmdb/itm_trees/public/models/%s/mdsplus", tokamak, version, version);
	*/
	//MERGE
    }
    else
...
// #define ASSUME_HOME_IN_AFS 1
        ...
        sprintf(homedir, "/afs/efda-itm.eu/user/%c/%s", user[0], user);
#else
        ...
        for (i = 0; i < sizeof(treeBases) / sizeof(treeBases[0]); ++i) {
            sprintf(treeBases[i],
                    "%s/public/itmdb/itm_trees/%s/%s/mdsplus/%d;/pfs/itmdb/itm_trees/public/models/%s/mdsplus",
...
//euitm_path env valus before calling getMdsShot
static char *prevEnv;
	...
	*/
	...
	if(name && translate)
	{
		...
		if(translatedBase && *translatedBase)	// the path to the ITM database is correctly set?
		{
			...
			prevEnv = prevLog;
			//MERGE
			if(prevLog)							// euitm_path is already set?
			{
			  //MERGE
				if (currPattern = strstr(prevLog,currTrans = getenv(baseName)))	// euitm_path already contains the correct path?
				{
			//Yes, move it to the head of the list
				...
				/*
				if (strstr(prevLog,getenv(baseName)))	// euitm_path already contains the correct path?
				{
					command=malloc(strlen(prevLog)+4);
					sprintf(command, "%s", prevLog);	// then it remains the same
				}
			...
			else							// euitm_path is not set yet
			{
...
void euitm_set_cache_level(int expIdx, int level)
{
    setCacheLevel(expIdx, level);
}
int euitm_get_cache_level(int expIdx)
{
    return getCacheLevel(expIdx);
}


void euitm_enable_mem_cache(int idx)
{
    euitm_set_cache_level(idx, 2);
    return;
}

 void euitm_disable_mem_cache(int idx)
{
//reportInfo("START DISABLE MEM CACHE\n", "");
    euitm_set_cache_level(idx, 0);
//reportInfo("END DISABLE MEM CACHE\n", "");
}

 void euitm_flush_mem_cache(int idx)
{
...
//reportInfo("END FLUSH MEM CACHE\n", "");
 ...
 void euitm_flush_cpo_mem_cache(int idx, char *cpoPath)
{
    if(getCacheLevel(idx) == 2)
    	flushCpoInfo(idx, cpoPath);
}

 void euitm_discard_mem_cache(int idx)
{
...
//reportInfo("END DISCARD MEM CACHE\n", "");
 ...
 void euitm_discard_cpo_mem_cache(int idx, char *cpoPath)
{
    if(getCacheLevel(idx) == 2)
...
static int checkExpIndex(int idx)
{
 ...
 int mdsEuitmOpenEnv(char *name, int shot, int run, int *retIdx, char *user, char *tokamak, char *version)
{
    ...
    {
    	status = mdsEuitmOpenEnvRemote(currConnectionId, name, shot, run, &remoteIdx, user, tokamak, version);
	if(status)
	    return status;
    ...
    lock("mdsEuitmOpenEnv");
    setDataEnv(user, tokamak, version);
    status = mdsEuitmOpen(expName, shot, run, retIdx);
    //MERGE
    restoreEnv(expName);
    //MERGE
    unlock();
...
int mdsEuitmConnect(char *ipAddr)
{
...
int mdsEuitmDisconnect()
{
 ...
 int mdsEuitmOpen(char *name, int shot, int run, int *retIdx)
{
	int status, i;
	void *ctx = 0;
	void *oldCtx;
	...
	checkCacheMonitor();
    	...
    	if(currConnectionId > 0)
    	{
    		status = mdsEuitmOpenRemote(currConnectionId, name, shot, run, &remoteIdx);
		if(status)
	    	    return status;
		currMdsShot = getMdsShot(name, shot, run, 0);
    		*retIdx = setExpIndexRemote(name, currMdsShot, currConnectionId, remoteIdx);
...
int mdsEuitmCreateEnv(char *name, int shot, int run, int refShot, int refRun, int *retIdx, char *user, char *tokamak, char *version)
{
    int status;
    int remoteIdx;
    int currMdsShot;
    ...
    if(currConnectionId > 0)
    {
    	status = mdsEuitmCreateEnvRemote(currConnectionId, name, shot, run, &remoteIdx, user, tokamak, version);
	if(status)
	    return status;
	currMdsShot = getMdsShot(name, shot, run, 0);
    	*retIdx = setExpIndexRemote(name, currMdsShot, currConnectionId, remoteIdx);
    ...
    lock("mdsEuitmCreateEnv");
    setDataEnv(user, tokamak, version);
    
     status = mdsEuitmCreate(expName, shot, run, refShot, refRun, retIdx);
    //MERGE
    restoreEnv(expName);
    //MERGE
    unlock();
 ...
 int mdsEuitmCreate(char *name, int shot, int run, int refShot, int refRun, int *retIdx)
{
	...
	checkCacheMonitor();
   	 ...
   	 if(currConnectionId > 0)
    	{
    	    status = mdsEuitmCreateRemote(currConnectionId, name, shot, run, &remoteIdx);
	    if(status)
	        return status;
	    currMdsShot = getMdsShot(name, shot, run, 0);
    	    *retIdx = setExpIndexRemote(name, currMdsShot, currConnectionId, remoteIdx);
...
int mdsEuitmClose(int idx, char *name, int shot, int run)
{
    int status;
    void *ctx;
    ...
    if(isExpRemote(idx) > 0)
    {
    	if(deleteExpIndex(idx, name, getMdsShot(name, shot, run, 0), &ctx)== 0)
    	    status = mdsEuitmCloseRemote(getExpConnectionId(idx), getExpRemoteIdx(idx), name, shot, run);
	else
	    status = 0;
	unlock();
	return status;
...
/*	printf("nid for %s %s = %d\n",cpoPath, path,nid);  */
	if(nid == -1)
	{
        ...
        if(!(status & 1))
        {
		sprintf(errmsg, "Missing data for CPO: %s, field: %s - %s", cpoPath, path, MdsGetMsg(status));
	    	unlock();
		return -1;
        }    
        if(numSegments > 0)
        {
//NOTE Segmented data encountered only when getDataLocal is called by getCpoDataServer in remote CPO access	
...
//if DTYPE_L it is a segment containing an object slice
	    ...
	    if(!(status & 1))
	    {
		sprintf(errmsg, "Cannot get segment limits for CPO: %s, field: %s - %s", cpoPath, path, MdsGetMsg(status));
		unlock();
		return -1;
	    }
    ...
    struct descriptor_xd *retTimesXd, int all)
{
    ...
    if(!(status & 1))
    {
	sprintf(errmsg, "Error getting number of segments for CPO: %s, field: %s - %s", cpoPath, path, MdsGetMsg(status));
	unlock();
	return -1;
    }
    if(numSegments == 0)
    ...
    for(currSegment = 0; currSegment < numSegments; currSegment++)
    {
        ...
        if(!(status & 1))
        {
            sprintf(errmsg, "Error getting segment limits for CPO: %s, field: %s", cpoPath, path);
	    unlock();
            return -1;
        }
        ...
        if(all || (currEnd >= start) || currSegment == numSegments - 1)
        {
            ...
            if(!(status & 1))
            {
                sprintf(errmsg, "Error getting segment for CPO: %s, field: %s", cpoPath, path);
	        unlock();
                return -1;
            }
            ...
            if(!(status & 1))
            {
                sprintf(errmsg, "Error getting segment time for CPO: %s, field: %s", cpoPath, path);
	        unlock();
                return -1;
            }
...
//reportInfo("PUT DATA LOCAL CALL PUT RECORD\n", path);
	...
	unlock();
 	if(!(status & 1))
	{
		sprintf(errmsg, "Cannot write data at path %s, CPO path %s: %s", path, cpoPath, MdsGetMsg(status));
		return -1;
	}
	return 0;
}
...
static int putSegmentLocal(int expIdx, char *cpoPath, char *path, char *timeBasePath, struct descriptor_a *dataD, double *times, int nTimes)
{
	...
	checkExpIndex(expIdx);
        ...
        len = strlen(path)+ 1;
        tempPath=malloc(len);
        strncpy(tempPath,convertPath(path),len);
        printf("tempPath = %s\n", tempPath);
        nid = getNid(convertPath(cpoPath), tempPath);
        printf("NID = %d\n", nid);
        free(tempPath);
	if(nid == -1)
	{
        ...
        if(!(status & 1))
	{
		sprintf(errmsg, "Cannot get num segments segment at path %s, CPO path %s: %s", path, cpoPath, MdsGetMsg(status));
		unlock();
		return -1;
	}
	len = strlen(timeBasePath)+ 1;
        tempPath=malloc(len);
    printf("ual_low_level_mdsplus : putSegmentLocal converted paths : %s , %s , %s\n", convertPath(cpoPath), convertPath(path), convertPath(timeBasePath));
        strncpy(tempPath,convertPath(timeBasePath),len);
        sprintf(segmentExpr, "data(GetCheckedSegment(build_path(\"%s:%s\"),%d))", convertPath(cpoPath),tempPath, currSegment);
 /*       printf("segmentExpr = %s\n",segmentExpr);  */
	...
	exprD.length = strlen(segmentExpr);
        ...
        if(!(status & 1))
	{
		sprintf(errmsg, "Internal error at path %s, CPO path %s: %s: cannot compile %s", path, cpoPath, MdsGetMsg(status), segmentExpr);
		unlock();
		return -1;
	}
        if(currSegment == 0)
        ...
        if(leftItems > 0)
        {
            ...
            if(!(status & 1))
            {
		sprintf(errmsg, "Cannot write data segment at path %s, CPO path %s: %s", path, cpoPath, MdsGetMsg(status));
		unlock();
		return -1;
            }
...
//printf("PUT SLICE LOCAL %s %s\n", path, decompileDsc(dataD));
	...
	if(nid == -1)
	{
        ...
        if(!(status & 1))
	{
		sprintf(errmsg, "Cannot get num segments at path %s, CPO path %s: %s", path, cpoPath, MdsGetMsg(status));
		unlock();
		return -1;
	}
//        sprintf(segmentExpr, "data(GetCheckedSegment(%s:time,%d))", convertPath(cpoPath), currSegment);
        sprintf(segmentExpr, "data(GetCheckedSegment(build_path(\"%s:time\"),%d))", convertPath(cpoPath), currSegment);
        exprD.length = strlen(segmentExpr);
        exprD.pointer = segmentExpr;
        status = TdiCompile(&exprD, &timeExprXd MDS_END_ARG);
        if(!(status & 1))
	{
        ...
        if(leftItems > 0)
        {
            ...
            {
		sprintf(errmsg, "Internal error in putSlice at path %s, CPO path %s: %s: wrong segment size", path, cpoPath, MdsGetMsg(status));
		unlock();
		return -1;
            }
           ...
           if(!(status & 1))
            {
		sprintf(errmsg, "Cannot write segment at path %s, CPO path %s: %s", path, cpoPath, MdsGetMsg(status));
		unlock();
		return -1;
            }
            if(leftBytes == dataLen) //if last data in segment, need to update end time
            {
                ...
                if(!(status & 1))
                {
                    sprintf(errmsg, "Cannot update segment at path %s, CPO path %s: %s", path, cpoPath, MdsGetMsg(status));
		    unlock();
                    return -1;
                }
            }    
		...
		unlock();
            ...
            if(!(status & 1))
            {
                sprintf(errmsg, "Cannot append data segment at path %s, CPO path %s: %s", path, cpoPath, MdsGetMsg(status));
		unlock();
                return -1;
            }
            free(dataPtr);
...
static int replaceLastSliceLocal(int expIdx, char *cpoPath, char *path, struct descriptor *dataD)
{
        ...
        if(!(status & 1))
	{
        ...
        if(currSegment == 0)
	{
		sprintf(errmsg, "No slice to be replaced %s, CPO path %s: %s", path, cpoPath, MdsGetMsg(status));
		unlock();
		return -1;
	}
#ifdef OLD_MDS
...
int mdsPutString(int expIdx, char *cpoPath, char *path, char *timeBasePath, char *inData)
{
    int status, i, data;
    EMPTYXD(emptyXd);
 ...
 int mdsPutFloat(int expIdx, char *cpoPath, char *path, char *timeBasePath, float data)
{
	int status;
 ...
 int mdsPutInt(int expIdx, char *cpoPath, char *path, char *timeBasePath, int data)
{
 ...
 int mdsPutDouble(int expIdx, char *cpoPath, char *path, char *timeBasePath, double data)
{
...
////////////////PUT SLICE ROUTINES
 ...
 int mdsPutStringSlice(int expIdx, char *cpoPath, char *path, char *data, double time)
{
	DESCRIPTOR_A(dataD, 1, DTYPE_BU, 0, 0);
	int status;   
        if(strlen(data) == 0)
	...
	dataD.pointer = data;
	status = putSlice(expIdx, cpoPath, path, (struct descriptor *)&dataD, time);
	return status;
}
 int mdsPutFloatSlice(int expIdx, char *cpoPath, char *path, float data, double time)
{
	struct descriptor dataD = {0, DTYPE_FLOAT, CLASS_S, 0};
	int status;   
	dataD.length = sizeof(float);
	dataD.pointer = (char *)&data;
	status = putSlice(expIdx, cpoPath, path, &dataD, time);
	return status;
}

 int mdsPutIntSlice(int expIdx, char *cpoPath, char *path, int data, double time)
{
	struct descriptor dataD = {0, DTYPE_L, CLASS_S, 0};
	int status;   
	dataD.length = sizeof(int);
 ...
 int mdsPutDoubleSlice(int expIdx, char *cpoPath, char *path, double data, double time)
{
	struct descriptor dataD = {0, DTYPE_DOUBLE, CLASS_S, 0};
	int status;   
	dataD.length = sizeof(double);
 ...
 int mdsPutVect1DIntSlice(int expIdx, char *cpoPath, char *path, int *data, int dim, double time)
{
	EMPTYXD(emptyXd);
	DESCRIPTOR_A(dataD, sizeof(int), DTYPE_L, 0, 0);
	int status;   
 ...
 int mdsPutVect1DDoubleSlice(int expIdx, char *cpoPath, char *path, double *data, int dim, double time)
{
	EMPTYXD(emptyXd);
	DESCRIPTOR_A(dataD, sizeof(double), DTYPE_DOUBLE, 0, 0);
	int status;   
 ...
 int mdsPutVect1DFloatSlice(int expIdx, char *cpoPath, char *path, float *data, int dim, double time)
{
	EMPTYXD(emptyXd);
	DESCRIPTOR_A(dataD, sizeof(float), DTYPE_FLOAT, 0, 0);
	int status;   
 ...
 int mdsPutVect2DIntSlice(int expIdx, char *cpoPath, char *path, int *data, int dim1, int dim2, double time)
{
	EMPTYXD(emptyXd);
	DESCRIPTOR_A_COEFF(dataD, sizeof(int), DTYPE_L, 0, 2, 0);
 	int status;   
 ...
 int mdsPutVect2DFloatSlice(int expIdx, char *cpoPath, char *path, float *data, int dim1, int dim2, double time)
{
	EMPTYXD(emptyXd);
	DESCRIPTOR_A_COEFF(dataD, sizeof(float), DTYPE_FLOAT, 0, 2, 0);
	int status;
 ...
 int mdsPutVect2DDoubleSlice(int expIdx, char *cpoPath, char *path, double *data, int dim1, int dim2, double time)
{
	EMPTYXD(emptyXd);
	DESCRIPTOR_A_COEFF(dataD, sizeof(double), DTYPE_DOUBLE, 0, 2, 0);
	int status;
 ...
 int mdsPutVect3DIntSlice(int expIdx, char *cpoPath, char *path, int *data, int dim1, int dim2, int dim3, double time)
{
	EMPTYXD(emptyXd);
	DESCRIPTOR_A_COEFF(dataD, sizeof(int), DTYPE_L, 0, 3, 0);
	int status;
 ...
 int mdsPutVect3DFloatSlice(int expIdx, char *cpoPath, char *path, float *data, int dim1, int dim2, int dim3, double time)
{
	EMPTYXD(emptyXd);
	DESCRIPTOR_A_COEFF(dataD, sizeof(float), DTYPE_FLOAT, 0, 3, 0);
	int status;
 ...
 int mdsPutVect3DDoubleSlice(int expIdx, char *cpoPath, char *path, double *data, int dim1, int dim2, int dim3, double time)
{
	EMPTYXD(emptyXd);
	DESCRIPTOR_A_COEFF(dataD, sizeof(double), DTYPE_DOUBLE, 0, 3, 0);
	int status;
 ...
 int mdsPutVect4DFloatSlice(int expIdx, char *cpoPath, char *path, float *data, int dim1, int dim2, int dim3, int dim4, double time)
{
	EMPTYXD(emptyXd);
 ...
 int mdsPutVect4DDoubleSlice(int expIdx, char *cpoPath, char *path, double *data, int dim1, int dim2, int dim3, int dim4, double time)
{
	EMPTYXD(emptyXd);
	DESCRIPTOR_A_COEFF(dataD, sizeof(double), DTYPE_DOUBLE, 0, 4, 0);
	int status;
 ...
 int mdsPutVect4DIntSlice(int expIdx, char *cpoPath, char *path, int *data, int dim1, int dim2, int dim3, int dim4, double time)
{
	EMPTYXD(emptyXd);
	DESCRIPTOR_A_COEFF(dataD, sizeof(double), DTYPE_L, 0, 4, 0);
	int status;
 ...
 int mdsPutVect5DFloatSlice(int expIdx, char *cpoPath, char *path, float *data, int dim1, int dim2, int dim3, 
    int dim4, int dim5, double time)
{
 ...
 int mdsPutVect5DDoubleSlice(int expIdx, char *cpoPath, char *path, double *data, int dim1, int dim2, int dim3, 
    int dim4, int dim5, double time)
{
	EMPTYXD(emptyXd);
	DESCRIPTOR_A_COEFF(dataD, sizeof(double), DTYPE_DOUBLE, 0, 5, 0);
 ...
 int mdsPutVect5DIntSlice(int expIdx, char *cpoPath, char *path, int *data, int dim1, int dim2, int dim3, 
    int dim4, int dim5, double time)
{
	EMPTYXD(emptyXd);
	DESCRIPTOR_A_COEFF(dataD, sizeof(double), DTYPE_L, 0, 5, 0);
...
/////6D Slice
 int mdsPutVect6DFloatSlice(int expIdx, char *cpoPath, char *path, float *data, int dim1, int dim2, int dim3, 
    int dim4, int dim5, int dim6, double time)
{
 ...
 int mdsPutVect6DDoubleSlice(int expIdx, char *cpoPath, char *path, double *data, int dim1, int dim2, int dim3, 
    int dim4, int dim5, int dim6, double time)
{
	EMPTYXD(emptyXd);
	DESCRIPTOR_A_COEFF(dataD, sizeof(double), DTYPE_DOUBLE, 0, 6, 0);
 ...
 int mdsPutVect6DIntSlice(int expIdx, char *cpoPath, char *path, int *data, int dim1, int dim2, int dim3, 
    int dim4, int dim5, int dim6, double time)
{
	EMPTYXD(emptyXd);
	DESCRIPTOR_A_COEFF(dataD, sizeof(double), DTYPE_L, 0, 6, 0);
 ...
 int mdsGetString(int expIdx, char *cpoPath, char *path, char **data)
{
	...
	if(!status) 
	{
		...
		{
			sprintf(errmsg, "Internal error: unexpected data type at CPO %s field %s", cpoPath, path);
			status = -1;
		}
 ...
 int mdsGetVect1DString(int expIdx, char *cpoPath, char *path, char  ***data, int *dim)
{
	...
	if(!status)
	{
		...
		{
			sprintf(errmsg, "Internal error: unexpected data type at CPO %s field %s.  Class= %d Type = %d\n", cpoPath, path,
...
static int mdsBeginCPOGetLocal(int expIdx, char *path, int isTimed, int *retSamples)
{
	...
	{
	    status = getData(expIdx, path, "time", &xd, 0);
//            status = getTimedData(expIdx, path, "time", 0., 0., &xd, &timesXd, 1);
	    if(status || !xd.l_length || xd.pointer->class != CLASS_A)
	    {
		sprintf(errmsg, "Internal error: Time not found for beginCPOGet at path %s: %s", path, MdsGetMsg(mdsStatus));
                mdsEndCPOGet(expIdx, path);
                return -1;
	    }
...
/*
	...
	{
		sprintf(errmsg, "Internal error: Time not found for beginCPOGetSlice at path %s: %s", path, MdsGetMsg(mdsStatus));
                mdsEndCPOGetSlice(expIdx, path);
		return -1;
	}
	arrayD = (struct descriptor_a *)xd.pointer;
	...
	if(arrayD->dtype == DTYPE_FLOAT)
	{
		...
		if(startIdx == totSamples)
		{
			...
			//sprintf(errmsg, "Specified time range outside recorded times for CPO %s", path);
			//MdsFree1Dx(&xd, 0);
                        //mdsEndCPOGetSlice(expIdx, path);
			//return -1;
		}
 ...
 int mdsGetIntSlice(int expIdx, char *cpoPath, char *path, char *timeBasePath, int *data, double time, double *retTime, int interpolMode)
{
	...
	if(status)
	{
		sprintf(errmsg, "Error reading CPO: %s, field: %s. %s", cpoPath, path, MdsGetMsg(mdsStatus));
		return -1;
	}
	if(status || !xd.l_length || xd.pointer->class != CLASS_A || xd.pointer->dtype != DTYPE_L)
	{
		sprintf(errmsg, "Error: not an int array at CPO:  %s, field %s", cpoPath, path);
		return -1;
	}
	arrayD = (struct descriptor_a *)xd.pointer;
 ...
 int mdsGetFloatSlice(int expIdx, char *cpoPath, char *path, char *timeBasePath, float *data, double time, double *retTime, int interpolMode)
{
	...
	{
		sprintf(errmsg, "Internal error: not a float array at CPO path %s, path %s", cpoPath, path);
		return -1;
	}
	arrayD = (struct descriptor_a *)xd.pointer;
 ...
 int mdsGetDoubleSlice(int expIdx, char *cpoPath, char *path, char *timeBasePath, double *data, double time, double *retTime, int interpolMode)
{
	...
	{
		sprintf(errmsg, "Internal error: not a double array at CPO path %s, path %s", cpoPath, path);
		return -1;
	}
	arrayD = (struct descriptor_a *)xd.pointer;
 ...
 int mdsGetVect1DIntSlice(int expIdx, char *cpoPath, char *path, char *timeBasePath, int **data, int *dim, double time, double *retTime, int interpolMode)
{
	...
	if(status || !xd.l_length || xd.pointer->class != CLASS_A || xd.pointer->dtype != DTYPE_L)
	{
		sprintf(errmsg, "Internal error: not a int array at CPO path %s, path %s", cpoPath, path);
		return -1;
	}
 ...
 int mdsGetVect2DFloatSlice(int expIdx, char *cpoPath, char *path, char *timeBasePath, float **data, int *dim1, int *dim2, double time, double *retTime, int interpolMode)
{
	...
	if(status)
	{
		sprintf(errmsg, "Error reading CPO: %s, field: %s. %s", cpoPath, path, MdsGetMsg(status));
		return -1;
	}
	if(!xd.l_length || xd.pointer->class != CLASS_A || xd.pointer->dtype != DTYPE_FLOAT)
	{
...
{
//Check whether the first path component is a number. In this case it referes to another CPO instance
    char *currPtr;
    int isNumeric;
...
static int putTimedObject(int expIdx, char *cpoPath, char *path, void *obj)
{
    ...
    if(apd->class != CLASS_APD)
    {
	sprintf(errmsg, "INTERNAL ERROR:Not an APD at path %s, CPO path %s", path, cpoPath);
	return -1;
    }
...
//Low Level CPO Copy. Instance Number -1 means default CPO
...
int mdsCopyCpo(int fromIdx, int toIdx, char *cpoName, int fromCpoOccur, int toCpoOccur)
{
    ...
    if(!(status & 1))
    {
        sprintf(errmsg, "Cannot find source CPO");
        return -1;
    }
    status = _TreeSetDefault(toCtx, toNameBuf, &toBaseNid);
    if(!(status & 1))
    {
        sprintf(errmsg, "Cannot find target CPO");
        return -1;
    }
...
void euitm_discard_mem(){}
void euitm_flush_all(){}
void euitm_flush(){}
void euitm_discard_old_mem(){}
