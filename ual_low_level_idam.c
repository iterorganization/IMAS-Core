...
int idamimasCreateEnv(char* name, int shot, int run, int refShot, int refRun, int* retIdx, char* user, char* tokamak,
                      char* version)
{
    return 0;
}
...
int idamimasCreate(char* name, int shot, int run, int refShot, int refRun, int* retIdx)
{
    ...
    sprintf(directive, "imas::create(file='%s', shot=%d, run=%d, refShot=%d, refRun=%d, /CreateFromModel, %s)", name,
            shot, run, refShot, refRun, keyword);
...
// Pass the Data
    ...
    sprintf(directive, "imas::get(expName='%s', idx=%d, group='%s', variable='%s', /getDimension, %s)", getExpName(idx),
            idx, cpoPath, path, keyword);
...
// Pass the Data
    ...
    sprintf(directive, "imas::put(expName='%s', idx=%d, group='%s', variable='%s', isTimed=%d, %s)", getExpName(idx),
            idx, cpoPath, path, isTimed, keyword);
...
static int
putDataX(int idx, char* cpoPath, char* path, int type, int nDims, int* dims, int dataOperation, void* data, double time)
{
...
// Pass the Data
    ...
    if (dataOperation == PUTSLICE_OPERATION) {
        sprintf(directive, "imas::put(expName='%s', idx=%d, group='%s', variable='%s', /putSlice, isTimed=0, %s)",
                getExpName(idx), idx, cpoPath, path, keyword);
...
// The time passed is set to Zero as it's the Last Time Slice that is to be replaced. Only the data is passed by a PUTDATABLOCK.
        sprintf(directive,
                "imas::put(expName='%s', idx=%d, group='%s', variable='%s', /replaceLastSlice, isTimed=0, %s)", idx,
                cpoPath, path, keyword);
...
// Pass the Data
    ...
    sprintf(directive, "imas::get(expName='%s', idx=%d, group='%s', variable='%s', type=%s, rank=%d, shot=%d, %s)",
            getExpName(idx), idx, cpoPath, path, data_type, rank, source, keyword);
...
static int getDataSlices(int idx, char* cpoPath, char* path, int type, int nDims, int* dims, char** data, double time,
...
// Pass the Data
    ...
    sprintf(directive,
            "imas::get(expName='%s', idx=%d, group='%s', variable='%s', type=%s, rank=%d, index1=%d, index2=%d, interpolMode=%d, /getSlice, %s)",
...
int
idamPutVect4DInt(int expIdx, char* cpoPath, char* path, int* data, int dim1, int dim2, int dim3, int dim4, int isTimed)
{
    int dims[4];
    dims[0] = dim1;
    dims[1] = dim2;
...
int idamPutVect4DFloat(int expIdx, char* cpoPath, char* path, float* data, int dim1, int dim2, int dim3, int dim4,
                       int isTimed)
{
    int dims[4];
    dims[0] = dim1;
...
int idamPutVect4DDouble(int expIdx, char* cpoPath, char* path, double* data, int dim1, int dim2, int dim3, int dim4,
                        int isTimed)
{
    int dims[4];
    dims[0] = dim1;
...
int idamPutVect5DInt(int expIdx, char* cpoPath, char* path, int* data, int dim1, int dim2, int dim3, int dim4, int dim5,
                     int isTimed)
{
    int dims[5];
    dims[0] = dim1;
...
int
idamPutVect5DFloat(int expIdx, char* cpoPath, char* path, float* data, int dim1, int dim2, int dim3, int dim4, int dim5,
                   int isTimed)
{
    int dims[5];
    dims[0] = dim1;
...
int idamPutVect5DDouble(int expIdx, char* cpoPath, char* path, double* data, int dim1, int dim2, int dim3, int dim4,
...
int
idamPutVect3DFloatSlice(int expIdx, char* cpoPath, char* path, float* data, int dim1, int dim2, int dim3, double time)
{
    int dims[3];
    dims[0] = dim1;
    dims[1] = dim2;
...
int
idamPutVect3DDoubleSlice(int expIdx, char* cpoPath, char* path, double* data, int dim1, int dim2, int dim3, double time)
{
    int dims[3];
    dims[0] = dim1;
    dims[1] = dim2;
...
int idamPutVect4DIntSlice(int expIdx, char* cpoPath, char* path, int* data, int dim1, int dim2, int dim3, int dim4,
                          double time)
{
    int dims[4];
    dims[0] = dim1;
...
int idamPutVect4DFloatSlice(int expIdx, char* cpoPath, char* path, float* data, int dim1, int dim2, int dim3, int dim4,
                            double time)
{
    int dims[4];
    dims[0] = dim1;
...
int
idamPutVect4DDoubleSlice(int expIdx, char* cpoPath, char* path, double* data, int dim1, int dim2, int dim3, int dim4,
                         double time)
{
    int dims[4];
    dims[0] = dim1;
...
int
idamReplaceLastVect4DIntSlice(int expIdx, char* cpoPath, char* path, int* data, int dim1, int dim2, int dim3, int dim4)
{
    int dims[4];
    dims[0] = dim1;
    dims[1] = dim2;
...
int idamReplaceLastVect4DFloatSlice(int expIdx, char* cpoPath, char* path, float* data, int dim1, int dim2, int dim3,
                                    int dim4)
{
    int dims[4];
    dims[0] = dim1;
...
int idamReplaceLastVect4DDoubleSlice(int expIdx, char* cpoPath, char* path, double* data, int dim1, int dim2, int dim3,
                                     int dim4)
{
    int dims[4];
    dims[0] = dim1;
...
int
idamGetVect4DDouble(int expIdx, char* cpoPath, char* path, double** data, int* dim1, int* dim2, int* dim3, int* dim4)
{
    int dims[4];
    int status;
...
int idamGetVect5DInt(int expIdx, char* cpoPath, char* path, int** data, int* dim1, int* dim2, int* dim3, int* dim4,
                     int* dim5)
{
    int dims[5];
    int status;
...
int
idamGetStringSlice(int expIdx, char* cpoPath, char* path, char** data, double time, double* retTime, int interpolMode)
{
...
int
idamGetFloatSlice(int expIdx, char* cpoPath, char* path, float* data, double time, double* retTime, int interpolMode)
{
    int dims[7];
    float* currData;
...
int
idamGetDoubleSlice(int expIdx, char* cpoPath, char* path, double* data, double time, double* retTime, int interpolMode)
{
    int dims[7];
    double* currData;
...
int idamGetVect1DIntSlice(int expIdx, char* cpoPath, char* path, int** data, int* dim, double time, double* retTime,
                          int interpolMode)
{
    int dims[7];
    int* currData;
...
int idamGetVect1DFloatSlice(int expIdx, char* cpoPath, char* path, float** data, int* dim, double time, double* retTime,
                            int interpolMode)
{
    int dims[7];
    float* currData;
...
int
idamGetVect1DDoubleSlice(int expIdx, char* cpoPath, char* path, double** data, int* dim, double time, double* retTime,
                         int interpolMode)
{
    int dims[7];
    double* currData;
...
int idamGetVect2DIntSlice(int expIdx, char* cpoPath, char* path, int** data, int* dim1, int* dim2, double time,
                          double* retTime, int interpolMode)
{
    int dims[7];
    int* currData;
...
int idamGetVect2DFloatSlice(int expIdx, char* cpoPath, char* path, float** data, int* dim1, int* dim2, double time,
                            double* retTime, int interpolMode)
{
    int dims[7];
    float* currData;
...
int idamGetVect2DDoubleSlice(int expIdx, char* cpoPath, char* path, double** data, int* dim1, int* dim2, double time,
                             double* retTime, int interpolMode)
{
    int dims[7];
    double* currData;
...
int
idamGetVect3DIntSlice(int expIdx, char* cpoPath, char* path, int** data, int* dim1, int* dim2, int* dim3, double time,
                      double* retTime, int interpolMode)
{
    int dims[7];
    int* currData;
...
int idamGetVect3DFloatSlice(int expIdx, char* cpoPath, char* path, float** data, int* dim1, int* dim2, int* dim3,
...
int idamGetVect3DDoubleSlice(int expIdx, char* cpoPath, char* path, double** data, int* dim1, int* dim2, int* dim3,
                             double time, double* retTime, int interpolMode)
{
    int dims[7];
    double* currData;
...
int idamEndCPOGet(int expIdx, char* path)
{
    return 0;
}

int idamEndCPOGetSlice(int expIdx, char* path)
{
    return 0;
}

int idamBeginCPOPut(int expIdx, char* path)
{
    return 0;
}

int idamEndCPOPut(int expIdx, char* path)
{
    return 0;
}

int idamBeginCPOPutTimed(int expIdx, char* path, int samples, double* inTimes)
{
    return 0;
}

int idamEndCPOPutTimed(int expIdx, char* path)
{
    return 0;
}

int idamBeginCPOPutNonTimed(int expIdx, char* path)
{
    return 0;
}

int idamEndCPOPutNonTimed(int expIdx, char* path)
{
    return 0;
}
...
int idamEndCPOPutSlice(int expIdx, char* path)
{
    return 0;
}
...
int idamEndCPOReplaceLastSlice(int expIdx, char* path)
{
    return 0;
}
...
// Pass the Data
    ...
    sprintf(directive, "imas::putObject(expName='%s', path='%s', index=%d, %s)", getExpName(index), path, index,
            keyword);
...
static int getDataSliceFromObject(void* obj, char* path, int index, int type, int nDims, int* dims, void** data)
{
    ...
    if (type == DIMENSION)
        sprintf(directive, "imas::getObject(expName='%s', path='%s', index=%d, objectId=%d, /getDimension, %s)",
...
// Pass the Data
    ...
    } else if (obj == (void*) -1) {            // Standard Object ID 1
        stdObjRef = (int*) malloc(sizeof(int));
        stdObjRef[0] = 1;
        putData[0].data = (void*) stdObjRef;
    } else {
    ...
    putData[1].count = 2;
    putData[1].shape = NULL;
    int* sliceIdx = (int*) malloc(
            putData[1].count * sizeof(int));            // Pass static variables from the application to the server
    ...
    sprintf(directive, "imas::beginObject(expName='%s', idx=%d, index=%d, relPath='%s', isTimed=%d, %s)",
            getExpName(index), expIdx, index, relPath, isTimed, keyword);
...
int idamGetObject(int expIdx, char* path, char* cpoPath, void** obj, int isTimed)
{
    ...
    sprintf(directive, "imas::getObjectGroup(expName='%s', idx=%d, path='%s', cpoPath='%s', isTimed=%d, %s)",
            getExpName(expIdx), expIdx, path, cpoPath, isTimed, keyword);
...
// Pass the Data
    ...
    double* sliceTime = (double*) malloc(
            putData[1].count * sizeof(double));    // Pass static variables from the application to the server
    ...
    sprintf(directive, "imas::getObjectSlice(expName='%s', idx=%d, cpoPath='%s', path='%s', %s)", getExpName(expIdx),
            expIdx, cpoPath, path, keyword);
...
// Pass the Data
    ...
    sprintf(directive, "imas::getObjectObject(expName='%s', index=%d, path='%s', %s)", getExpName(index), index, path,
            keyword);
...
// int mdsPutObject(int expIdx, char *cpoPath, char *path, void *obj, int isTimed);
    ...
    sprintf(directive,
            "imas::putObjectGroup(expName='%s', idx=%d, cpoPath='%s', path='%s', objectId=%d, isTimed=%d, %s)",
            getExpName(currIdx), currIdx, cpoPath, path, ((int*) obj)[0], isTimed, keyword);
...
void*
idamPutVect5DIntInObject(void* obj, char* path, int idx, int* data, int dim1, int dim2, int dim3, int dim4, int dim5)
{
    int dim[5] = { dim1, dim2, dim3, dim4, dim5 };
    return putDataSliceInObject(obj, path, idx, INT, 5, dim, (void*) data);
}

void* idamPutVect5DFloatInObject(void* obj, char* path, int idx, float* data, int dim1, int dim2, int dim3, int dim4,
                                 int dim5)
{
    int dim[5] = { dim1, dim2, dim3, dim4, dim5 };
...
void* idamPutVect5DDoubleInObject(void* obj, char* path, int idx, double* data, int dim1, int dim2, int dim3, int dim4,
                                  int dim5)
{
    int dim[5] = { dim1, dim2, dim3, dim4, dim5 };
...
void*
idamPutVect6DIntInObject(void* obj, char* path, int idx, int* data, int dim1, int dim2, int dim3, int dim4, int dim5,
                         int dim6)
{
...
void* idamPutVect6DFloatInObject(void* obj, char* path, int idx, float* data, int dim1, int dim2, int dim3, int dim4,
                                 int dim5, int dim6)
{
    int dim[6] = { dim1, dim2, dim3, dim4, dim5, dim6 };
...
void* idamPutVect6DDoubleInObject(void* obj, char* path, int idx, double* data, int dim1, int dim2, int dim3, int dim4,
                                  int dim5, int dim6)
{
    int dim[6] = { dim1, dim2, dim3, dim4, dim5, dim6 };
...
void*
idamPutVect7DIntInObject(void* obj, char* path, int idx, int* data, int dim1, int dim2, int dim3, int dim4, int dim5,
                         int dim6, int dim7)
{
...
void* idamPutVect7DFloatInObject(void* obj, char* path, int idx, float* data, int dim1, int dim2, int dim3, int dim4,
                                 int dim5, int dim6, int dim7)
{
    int dim[7] = { dim1, dim2, dim3, dim4, dim5, dim6, dim7 };
...
void* idamPutVect7DDoubleInObject(void* obj, char* path, int idx, double* data, int dim1, int dim2, int dim3, int dim4,
                                  int dim5, int dim6, int dim7)
{
    int dim[7] = { dim1, dim2, dim3, dim4, dim5, dim6, dim7 };
...
// Pass the Data
    ...
    sprintf(directive, "imas::putObjectInObject(expName='%s', index=%d, path='%s', %s)", getExpName(idx), idx, path,
            keyword);
...
int idamGetDimensionFromObject(int expIdx, void* obj, char* path, int idx, int* nDims, int* dim1, int* dim2, int* dim3,
                               int* dim4, int* dim5, int* dim6, int* dim7)
{
    int dims[7];
    int status;
...
int
idamGetVect4DIntFromObject(void* obj, char* idamPath, int idx, int** data, int* dim1, int* dim2, int* dim3, int* dim4)
{
    int dim[4];
...
int idamGetVect4DFloatFromObject(void* obj, char* idamPath, int idx, float** data, int* dim1, int* dim2, int* dim3,
                                 int* dim4)
{
    int dim[4];
...
int idamGetVect4DDoubleFromObject(void* obj, char* idamPath, int idx, double** data, int* dim1, int* dim2, int* dim3,
                                  int* dim4)
{
    int dim[4];
...
int
idamGetVect5DIntFromObject(void* obj, char* idamPath, int idx, int** data, int* dim1, int* dim2, int* dim3, int* dim4,
                           int* dim5)
{
    int dim[5];
...
int idamGetVect5DFloatFromObject(void* obj, char* idamPath, int idx, float** data, int* dim1, int* dim2, int* dim3,
                                 int* dim4, int* dim5)
{
    int dim[5];
...
int idamGetVect5DDoubleFromObject(void* obj, char* idamPath, int idx, double** data, int* dim1, int* dim2, int* dim3,
                                  int* dim4, int* dim5)
{
    int dim[5];
...
int
idamGetVect6DIntFromObject(void* obj, char* idamPath, int idx, int** data, int* dim1, int* dim2, int* dim3, int* dim4,
                           int* dim5, int* dim6)
{
    int dim[6];
...
int idamGetVect6DFloatFromObject(void* obj, char* idamPath, int idx, float** data, int* dim1, int* dim2, int* dim3,
                                 int* dim4, int* dim5, int* dim6)
{
    int dim[6];
...
int idamGetVect6DDoubleFromObject(void* obj, char* idamPath, int idx, double** data, int* dim1, int* dim2, int* dim3,
                                  int* dim4, int* dim5, int* dim6)
{
    int dim[6];
...
int
idamGetVect7DIntFromObject(void* obj, char* idamPath, int idx, int** data, int* dim1, int* dim2, int* dim3, int* dim4,
                           int* dim5, int* dim6, int* dim7)
{
    int dim[7];
...
int idamGetVect7DFloatFromObject(void* obj, char* idamPath, int idx, float** data, int* dim1, int* dim2, int* dim3,
                                 int* dim4, int* dim5, int* dim6, int* dim7)
{
    int dim[7];
...
int idamGetVect7DDoubleFromObject(void* obj, char* idamPath, int idx, double** data, int* dim1, int* dim2, int* dim3,
                                  int* dim4, int* dim5, int* dim6, int* dim7)
{
    int dim[7];
...
// Pass the Data
    ...
    sprintf(directive, "imas::putObjectSlice(expName='%s', idx=%d, cpoPath='%s', path='%s', objectId=%d, %s)",
            getExpName(expIdx), expIdx, cpoPath, path, ((int*) obj)[0], keyword);
...
// int mdsReplaceLastObjectSlice(int expIdx, char *cpoPath, char *path, void *obj)
    ...
    sprintf(directive, "imas::replaceLastObjectSlice(expName='%s', idx=%d, cpoPath='%s', path='%s', objectId=%d, %s)",
            getExpName(expIdx), expIdx, cpoPath, path, ((int*) obj)[0], keyword);
...
int idambeginIdsGet(int expIdx, char* path, int isTimed, int* retSamples)
{
    *retSamples = 0;
    ...
    sprintf(directive, "imas::beginIdsGet(expName='%s', idx=%d, path=%s, isTimed=%d, /imas_mds)", getExpName(expIdx),
            expIdx, path, isTimed);
...
// Pass the Data
    ...
    sprintf(directive, "imas::beginIdsGetSlice(expName='%s', idx=%d, path='%s', /imas_mds)", getExpName(expIdx), expIdx,
            path);
...
int idamendIdsGetSlice(int currIdx, char* path)
{
    ...
    sprintf(directive, "imas::endIdsGetSlice(expName='%s', idx=%d, path=%s, /imas_mds)", getExpName(currIdx), currIdx,
            path);
...
int idambeginIdsPutSlice(int expIdx, char* path)
{
    ...
    sprintf(directive, "imas::beginIdsPutSlice(expName='%s', idx=%d, path=%s, /imas_mds)", getExpName(expIdx), expIdx,
            path);
...
int idamendIdsPutSlice(int expIdx, char* path)
{
    ...
    sprintf(directive, "imas::endIdsPutSlice(expName='%s', idx=%d, path=%s, /imas_mds)", getExpName(expIdx), expIdx,
            path);
...
int idamendIdsPutNonTimed(int expIdx, char* path)
{
    ...
    sprintf(directive, "imas::endIdsPutNonTimed(expName='%s', idx=%d, path=%s, /imas_mds)", getExpName(expIdx), expIdx,
            path);
...
int idambeginIdsPutNonTimed(int expIdx, char* path)
{
    ...
    sprintf(directive, "imas::beginIdsPutNonTimed(expName='%s', idx=%d, path=%s, /imas_mds)", getExpName(expIdx),
            expIdx, path);
...
int idamendIdsReplaceLastSlice(int expIdx, char* path)
{
    ...
    sprintf(directive, "imas::endIdsReplaceLastSlice(expName='%s', idx=%d, path=%s, /imas_mds)", getExpName(expIdx),
            expIdx, path);
...
int idambeginIdsReplaceLastSlice(int expIdx, char* path)
{
    ...
    sprintf(directive, "imas::beginIdsReplaceLastSlice(expName='%s', idx=%d, path=%s, /imas_mds)", getExpName(expIdx),
            expIdx, path);
...
// Pass the Data
    ...
    sprintf(directive, "imas::beginIdsPutTimed(expName='%s', idx=%d, path='%s', /imas_mds)", getExpName(expIdx), expIdx,
            path);
...
int idamendIdsPutTimed(int expIdx, char* path)
{
    ...
    sprintf(directive, "imas::endIdsPutTimed(expName='%s', idx=%d, path=%s, /imas_mds)", getExpName(expIdx), expIdx,
            path);
