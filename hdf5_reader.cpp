...
:backend_version(backend_version_), tensorized_paths(), opened_data_sets (), opened_shapes_data_sets(), shapes_data(), shapes_selection_readers(),
...
void
HDF5Reader::openPulse (PulseContext * ctx, int mode, std::string & options,
		       std::string & backend_version, hid_t * file_id,
		       std::unordered_map < std::string,
		       hid_t > &opened_IDS_files, int files_paths_strategy, std::string &files_directory, std::string &relative_file_path)
{
  HDF5Utils hdf5_utils;
  std::string filePath =
    hdf5_utils.pulseFilePathFactory (ctx, files_paths_strategy,
				     files_directory, relative_file_path);
  ...
  od.count = 0;
  H5Literate (*file_id, H5_INDEX_NAME, H5_ITER_NATIVE, NULL, file_info,
	      (void *) &od);
  ...
  if (H5Aexists (*file_id, backend_version_attribute_name) > 0)
    {
      hid_t att_id =
	H5Aopen (*file_id, backend_version_attribute_name, H5P_DEFAULT);
      if (att_id < 0)
	{
      ...
      if (tset < 0)
	{
	  ...
	  sprintf (error_message,
		   "Unable to set characters to UTF8 for: %s\n",
		   backend_version_attribute_name);
  ...
  else
    {
      char error_message[200];
      sprintf (error_message,
	       "Not a IMAS HDF5 pulse file. Unable to find attribute: %s\n",
	       backend_version_attribute_name);
      throw UALBackendException (error_message, LOG);
    }
}
...
void
HDF5Reader::closePulse (PulseContext * ctx, int mode, std::string & options,
			hid_t file_id, std::unordered_map < std::string,
			hid_t > &opened_IDS_files, int files_path_strategy, std::string &files_directory, std::string &relative_file_path)
{
  close_datasets ();
  herr_t status = H5Fclose (file_id);
  HDF5Utils hdf5_utils;
   std::string filePath =
    hdf5_utils.pulseFilePathFactory (ctx, files_path_strategy,
				     files_directory, relative_file_path);
...
{
  std::unordered_map < std::string, hid_t >::iterator it =
    opened_data_sets.begin ();
  while (it != opened_data_sets.end ())
    {
      hid_t dset_id = it->second;
      H5Dclose (dset_id);
      it++;
    }
...
void
HDF5Reader::close_dataset (hid_t dataset_id, std::string &tensorized_path, hid_t dataset_id_shapes)
{
...
void
HDF5Reader::beginReadArraystructAction (ArraystructContext * ctx, int *size)
{
  if (IDS_group_id == -1)
...
int
HDF5Reader::getSliceIndex (OperationContext * opCtx, std::string & att_name,
			   std::string & timebasename, int *slice_sup,
			   double *linear_interpolation_factor, int timed_AOS_index)
{
  ...
  if (homogeneous_time == 1)
	...
	{
	  if (timebasename.compare ("&time") == 0
	      || timebasename.compare ("time") == 0)
	    {
	      dataset_name = "time";
	    }
	  else
  ...
  if (opened_data_sets.find (dataset_name) != opened_data_sets.end ())
    ...
    {
      dataset_id =
	H5Dopen2 (IDS_group_id, dataset_name.c_str (), H5P_DEFAULT);
  ...
  if (dataset_id < 0)
    {
      char error_message[200];
      sprintf (error_message,
	       "Unable to open dataset: %s for getting time vector.\n",
	       dataset_name.c_str ());
      throw UALBackendException (error_message, LOG);
    }
  ...
  int dataset_rank =
    H5Sget_simple_extent_dims (dataspace, dataspace_dims, NULL);
  H5Sclose (dataspace);
  if (dataset_rank < 0)
    {
      char error_message[200];
      sprintf (error_message,
	       "Unable to call H5Sget_simple_extent_dims on dataset:%s\n",
	       dataset_name.c_str ());
      throw UALBackendException (error_message, LOG);
    }
  ...
  if (homogeneous_time == 1 || dataset_name.compare ("time") == 0)
    {
      slices_times =
	(double *) malloc (dataspace_dims[dataset_rank - 1] *
			   sizeof (H5T_NATIVE_DOUBLE));
      herr_t status =
	H5Dread (dataset_id, H5T_NATIVE_DOUBLE, H5P_DEFAULT, H5P_DEFAULT,
		 H5P_DEFAULT, (void *) slices_times);
      if (status < 0)
	{
	  char error_message[200];
	  sprintf (error_message,
		   "Unable to read the homogeneous time basis:%s\n",
		   dataset_name.c_str ());
	  throw UALBackendException (error_message, LOG);
	}
    }
  else
    {
     ...
     HDF5HsSelectionReader hsSelectionReader (dataset_id,
			   ualconst::double_data,
			   current_arrctx_indices.size(), 
			   &dim);
      ...
      if (status < 0)
	{
	  char error_message[200];
	  sprintf (error_message,
		   "Unable to read the inhomogeneous time basis: %s\n",
		   dataset_name.c_str ());
	  throw UALBackendException (error_message, LOG);
	}
    }
  ...
  *slice_sup = dataspace_dims[dataset_rank - 1] - 1;
  ...
  switch (interp)
    ...
    case CLOSEST_INTERP:
      ...
      if (fabs (time - slices_times[*slice_sup]) <=
	  (fabs (time - slices_times[slice_inf])))
	{
	  closest = *slice_sup;
	}
      else
      ...
      else
	{
      ...
      *linear_interpolation_factor =
	(time - slices_times[slice_inf]) / (slices_times[*slice_sup] -
					    slices_times[slice_inf]);
      free (slices_times);
      return slice_inf;
      break;
...
int
HDF5Reader::read_ND_Data (Context * ctx, std::string & att_name,
			  std::string & timebasename, int datatype,
			  void **data, int *dim, int *size)
{
  ...
  if (non_existing_data_sets.find (tensorized_path) !=
      non_existing_data_sets.end ())  //optimization
    {				
      return 0;
    }
  ...
  else if (H5Lexists (IDS_group_id, tensorized_path.c_str (), H5P_DEFAULT) ==
	   0)
    {
      non_existing_data_sets[tensorized_path] = 1;
      return 0;
  ...
  if (ctx->getType () == CTX_OPERATION_TYPE
      ...
      if (opCtx->getRangemode () == SLICE_OP)
	{
	  ...
	  bool search_slice_index = (slice_mode == SLICE_OP) && (is_dynamic
								 || isTimed);
	  if (search_slice_index)
	    {
	      slice_index =
		getSliceIndex (opCtx, dataset_name, timebasename, &slice_sup,
			       &linear_interpolation_factor, timed_AOS_index);
	    }
	}
    }
  ...
  if (selection_readers.find (tensorized_path) != selection_readers.end ())
    {
      HDF5HsSelectionReader & hsSelectionReader =
	*selection_readers[tensorized_path];
      ...
      if (shapesDataSetExists == 1)
	{
      ...
      hsSelectionReader.setHyperSlabs (slice_mode, is_dynamic,
				       isTimed, slice_index,
				       timed_AOS_index,
      ...
      hsSelectionReader.allocateBuffer (data, slice_mode, is_dynamic, isTimed,
					slice_index);
    ...
    {
      std::unique_ptr < HDF5HsSelectionReader >
	hsSelectionReader (new
			   HDF5HsSelectionReader (dataset_id, datatype,
						  current_arrctx_indices.size
						  (), dim));
      ...
      if (shapesDataSetExists == 1)
	{
      ...
      hsSelectionReader->getSize (size, slice_mode, is_dynamic);
      hsSelectionReader->allocateBuffer (data, slice_mode, is_dynamic,
					 isTimed, slice_index);
      ...
      if (datatype != ualconst::char_data)
	{
	  status =
	    H5Dread (dataset_id, hsSelectionReader->dtype_id,
		     hsSelectionReader->memspace,
		     hsSelectionReader->dataspace, H5P_DEFAULT, *data);
	}
      else
	{
	  status =
  ...
  if (!ignore_linear_interpolation)
	...
	*selection_readers[tensorized_path];
      ...
      hsSelectionReader.allocateBuffer (&next_slice_data, slice_mode,
					is_dynamic, isTimed, slice_sup);


      status =
	H5Dread (dataset_id, hsSelectionReader.dtype_id,
		 hsSelectionReader.memspace, hsSelectionReader.dataspace,
		 H5P_DEFAULT, next_slice_data);
	...
	case ualconst::integer_data:
	  {
	    ...
	    for (size_t i = 0; i < N; i++)
	      data_int[i] =
		(int) round (data_int[i] +
			     (next_slice_data_int[i] -
			      data_int[i]) * linear_interpolation_factor);
	...
	case ualconst::double_data:
	  {
	    ...
	    for (size_t i = 0; i < N; i++)
	      data_double[i] =
		data_double[i] + (next_slice_data_double[i] -
				  data_double[i]) *
		linear_interpolation_factor;
	    break;
	  }
	case ualconst::char_data:
	  {
	    char error_message[200];
	    sprintf (error_message,
		     "Linear interpolation of char data not supported, dataset: %s\n",
		     tensorized_path.c_str ());
	    throw UALBackendException (error_message, LOG);
	    break;
	  }
...
int
HDF5Reader::getPersistentShapes (Context * ctx,
				 const std::string & tensorized_path,
				 int datatype,
				 int slice_mode,
				 bool is_dynamic,
				 bool isTimed,
				 int slice_index, int dim, int *size, bool *zero_shape, hid_t *dataset_id_shapes)
{
  ...
  if ((datatype != ualconst::char_data && dim > 0)
      || (datatype == ualconst::char_data && dim == 2))
    {
      int *shapes = nullptr;
      shapesDataSetExists =
	readPersistentShapes (ctx, tensorized_path, (void **) &shapes,
			      slice_mode, is_dynamic, isTimed, slice_index, zero_shape, dataset_id_shapes);
...
HDF5Reader::readPersistentShapes (Context * ctx,
				  ...
				  int slice_index, bool *zero_shape, hid_t *dataset_id_shapes)
{
...
void
HDF5Reader::open_IDS_group (OperationContext * ctx, hid_t file_id, std::unordered_map < std::string,
			      hid_t > &opened_IDS_files, std::string &files_directory, std::string &relative_file_path)
{
  ...
  hid_t IDS_file_id = opened_IDS_files[IDS_link_name];
   if (IDS_file_id == -1) {
      std::string IDS_pulse_file = hdf5_utils.getIDSPulseFilePath (files_directory, relative_file_path,
				  IDS_link_name);
      ...
      if (IDS_file_id < 0)
	{
	  char error_message[200];
	  sprintf (error_message,
		  "unable to open external file for IDS: %s.\n",
  ...
  hid_t loc_id =
    hdf5_utils.openHDF5Group (ctx->getDataobjectName ().c_str (), file_id);
...
void HDF5Reader::close_file_handler(std::string external_link_name, std::unordered_map < std::string,
				    hid_t > &opened_IDS_files) {
