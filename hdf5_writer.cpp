...
:  backend_version(backend_version_), tensorized_paths(), opened_data_sets(), dataset_handlers(), selection_writers(), homogeneous_time(-1), current_arrctx_indices(), current_arrctx_shapes(), IDS_group_id(-1), init_slice_index(false), dynamic_aos_already_extended_by_slicing(), dynamic_AOS_slices_extension(0), use_core_driver(false), slice_mode(GLOBAL_OP)
{
...
void HDF5Writer::create_file_in_memory(std::string idsName, const std::string & coreFileName, std::unordered_map < std::string, hid_t > &opened_IDS_files, hbool_t flush)
{
    ...
    assert(H5Pset_fapl_core(fapl, 1024*500*1024, flush) >= 0);
...
void HDF5Writer::write_ND_Data(Context * ctx, hid_t loc_id, std::string & att_name, std::string & timebasename, int datatype, int dim, int *size, void *data)
{
    ...
    if (slice_mode != SLICE_OP) {
        ...
        if (dataset_id < 0)     //in global mode (put), data set not yet created
        {
            ...
            dataSetHandler->createOrOpenTensorizedDataSet2(tensorized_path.c_str(), datatype, dim, size, loc_id, &dataset_id, AOSRank, AOSShapes.data(), 1, false, timed_AOS_index, compression_enabled);
        ...
        {
            bool create_data_set = H5Lexists(loc_id, tensorized_path.c_str(), H5P_DEFAULT) <= 0;
            std::unique_ptr < HDF5DataSetHandler > dataSetHandler(new HDF5DataSetHandler());
            dataSetHandler->setSliceMode(ctx, homogeneous_time);
            dataSetHandler->createOrOpenTensorizedDataSet2(tensorized_path.c_str(), datatype, dim, size, loc_id, &dataset_id, AOSRank, AOSShapes.data(), create_data_set, false, timed_AOS_index, compression_enabled);

            dataSetHandler->updateTensorizedDataSet(ctx, tensorized_path, datatype, dim, size, loc_id, &dataset_id, AOSRank, AOSShapes.data(), current_arrctx_indices, dynamic_aos_already_extended_by_slicing, false, dynamic_AOS_slices_extension);        //updating dataset_extended
    ...
    if ((datatype != ualconst::char_data && dim > 0)
        ...
        dataset_shape_id = createOrUpdateShapesDataSet(ctx, loc_id, tensorized_path, dataSetHandler, timebasename, timed_AOS_index);
...
std::string & timebasename, int timed_AOS_index)
{
    ...
    {
		create_data_set = slice_mode != SLICE_OP;
        std::unique_ptr < HDF5DataSetHandler > dataSetHandler(new HDF5DataSetHandler());
		...
		for (int i = 0; i < rank - AOSRank; i++) {
            shapes[i] = dataspace_dims[i + AOSRank];   //previous size before extension
        }

        dataSetHandler->createOrOpenTensorizedDataSet(tensorized_path.c_str(), ualconst::integer_data, dim, size, loc_id, &dataset_id, AOSRank, aos_shapes.data(), create_data_set, true, timed_AOS_index, compression_enabled);

        if (slice_mode == SLICE_OP && timed_AOS_index == -1) {
            //dataSetHandler->setSliceInitialisation();
			//if (timed_AOS_index == -1)
				shapes[rank - AOSRank -1] = dataspace_dims[rank -1] + fieldHandler.getSlicesExtension();
        }
    ...
    } else                      //dataset already used in previous LL request
    {
        ...
        hsize_t* dataspace_dims_before_extension = fieldHandler.getDims();

        for (int i = 0; i < rank - AOSRank; i++) {
            shapes[i] = dataspace_dims_before_extension[i + AOSRank];   //previous size before extension
			if (slice_mode == SLICE_OP && i == rank - AOSRank - 1 && timed_AOS_index == -1) {
				shapes[i] = dataspace_dims_before_extension[i + AOSRank] + fieldHandler.getSlicesExtension();
            }
        }

        //if (slice_mode == SLICE_OP)
        //    dataSetHandler.setSliceInitialisation();
        dataSetHandler.updateTensorizedDataSet(ctx, tensorized_path, ualconst::integer_data, dim, size, loc_id, &dataset_id, AOSRank, aos_shapes.data(), aos_indices, dynamic_aos_already_extended_by_slicing, true, dynamic_AOS_slices_extension); //updating AOS_SHAPE dataset
...
void HDF5Writer::createOrUpdateAOSShapesDataSet(Context * ctx, hid_t loc_id, std::string & IDS_link_name)
{
    ...
    if (slice_mode == SLICE_OP && timed_AOS_index != -1 && timed_AOS_index < AOSRank) { //slicing is inside a dynamic AOS
		aos_shapes[timed_AOS_index] += dynamic_AOS_slices_extension;
    }

    if (slice_mode == SLICE_OP) {
        int AOS_shape_value = 0;
        if (timed_AOS_index == AOSRank)
			AOS_shape_value = current_arrctx_shapes[timed_AOS_index] + dynamic_AOS_slices_extension;
    ...
    if (H5Lexists(loc_id, tensorized_path.c_str(), H5P_DEFAULT) > 0) {  //not yet used by a previous LL request
        ...
        if (dataset_id < 0) {
            ...
            dataSetHandler->createOrOpenTensorizedDataSet(tensorized_path.c_str(), ualconst::integer_data, dim, size, loc_id, &dataset_id, AOSRank, aos_shapes.data(), 0, true, timed_AOS_index, compression_enabled);
            dataSetHandler->updateAOSShapesTensorizedDataSet(ctx, tensorized_path, ualconst::integer_data, dim, size, loc_id, &dataset_id, AOSRank, aos_shapes.data());
            HDF5HsSelectionWriter hsSelectionWriter;
            hsSelectionWriter.setHyperSlabs(dataset_id, aos_indices, GLOBAL_OP, *dataSetHandler, dynamic_AOS_slices_extension);
            status = H5Dwrite(dataset_id, dataSetHandler->dtype_id, hsSelectionWriter.memspace, dataSetHandler->getDataSpace(), H5P_DEFAULT, shapes);
        ...
        dataSetHandler->createOrOpenTensorizedDataSet(tensorized_path.c_str(), ualconst::integer_data, dim, size, loc_id, &dataset_id, AOSRank, aos_shapes.data(), 1, true, timed_AOS_index, compression_enabled);
