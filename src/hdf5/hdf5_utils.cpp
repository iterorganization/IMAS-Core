...
int HDF5Utils::openPulse(DataEntryContext *ctx, int mode, std::string &backend_version, hid_t *file_id, std::unordered_map<std::string, hid_t> &opened_IDS_files, int files_paths_strategy, std::string &files_directory, std::string &relative_file_path, std::string &pulseFilePath)
{

    HDF5Utils hdf5_utils;

    bool open_read_only;
    hdf5_utils.readOption(ctx->getURI(), &open_read_only);
    ...
    if (!(mode == OPEN_PULSE || mode == FORCE_OPEN_PULSE))
        throw ALBackendException("HDF5Backend: unexpected mode in HDF5Utils::openPulse()", LOG);
    ...
    case FORCE_OPEN_PULSE:
        if (!exists(pulseFilePath.c_str()))
        {
            if (open_read_only)
            {
                ...
                sprintf(error_message, "unable to open a non existing file in Read only (open_read_only='yes').\n");
...
void HDF5Utils::createPulse(DataEntryContext *ctx, int mode, std::string backend_version, hid_t *file_id, std::unordered_map<std::string, hid_t> &opened_IDS_files, int files_paths_strategy, std::string &files_directory, std::string &relative_file_path, std::string &pulseFilePath)
{
    HDF5Utils hdf5_utils;
    ...
    {
        char error_message[200];
        sprintf(error_message, "unable to create a pulse file when open_read_only='yes'.\n");
        throw ALBackendException(error_message, LOG);
    }
...
void HDF5Utils::openMasterFile(DataEntryContext *ctx, hid_t *file_id, const std::string &filePath)
{ //open master file
...
void HDF5Utils::open_IDS_group(OperationContext *ctx, hid_t file_id, std::unordered_map<std::string, hid_t> &opened_IDS_files, std::string &files_directory, std::string &relative_file_path, hid_t *IDS_group_id)
{
