...
int DataInterpolation::getSlicesTimesIndices(double requested_time, const std::vector<double> &time_vector, std::map<std::string, int> &times_indices, int interp) {
    ...
    if (interp == -1 || (interp != CLOSEST_INTERP && interp != PREVIOUS_INTERP && interp != LINEAR_INTERP)) {
    ...
    switch (interp) {
        ...
        case PREVIOUS_INTERP:
            {
                closest = slice_inf;
                if (slice_sup > 0 && ( (fabs(requested_time - time_vector[slice_sup]) < std::numeric_limits<double>::epsilon()) 
                                    || (requested_time > time_vector[slice_sup])) )
                    closest = slice_sup;
                return closest;
                break;
            }
        ...
        case LINEAR_INTERP:
            {
                ...
                //printf("slice_inf=%d, linear interp\n", slice_inf);
                return slice_inf;
                break;
            }
    }
...
std::map<std::string, double> &slices_times, double requested_time, void **result, int interp) {
    ...
    if ((slices_times.size() != 2) || (y_slices.size() != 2)) 
        throw ALBackendException("Exactly 2 slices are currently required for the LINEAR_INTERP interpolation mode.", LOG);
        ...
        case alconst::double_data:
            {
                ...
                if (interpolation_factor != 0) {
                    for (size_t i = 0; i < shape; i++) 
                        data_double[i] += (next_slice_data_double[i] - data_double[i]) * interpolation_factor;
                }
                if (!same_slices)
